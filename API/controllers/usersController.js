 ...
 *   patch:
 *     tags: [Usuaris]
 *     summary: Marcar el tutorial com completat
 *     description: Actualitza el camp `tutorial` a FALSE per indicar que l'usuari ja ha completat el tutorial.
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
 ...
 *     responses:
 *       200:
 *         description: Estat del tutorial actualitzat
 *       404:
 *         description: No s'ha trobat cap usuari amb aquest ID
 *       500:
 *         description: Error en la consulta
 */
exports.marcarTutorialCompletat = async (req, res) => {
    try {
        const connection = await connectDB();
        const usuariId = req.params.id;

        const [result] = await connection.execute(
            'UPDATE USUARIS SET tutorial = FALSE WHERE id = ?',
            [usuariId]
        );
        ...
        res.status(200).send({ missatge: "Tutorial marcat com completat correctament." });
    } catch (err) {
        console.error(err);
        res.status(500).send("Error al marcar el tutorial com completat.");
    }
};