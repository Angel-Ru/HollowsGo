...
// POST /amics/afegir
exports.afegirAmic = async (req, res) => {
    try {
        const { id_usuari, id_usuari_amic } = req.body;
        const connection = await connectDB();

        // Evitar amistats duplicades (A-B o B-A)
        const [existeix] = await connection.execute(`
      SELECT * FROM AMISTATS
      WHERE (id_usuari = ? AND id_usuari_amic = ?)
         OR (id_usuari = ? AND id_usuari_amic = ?)
    `, [id_usuari, id_usuari_amic, id_usuari_amic, id_usuari]);

        if (existeix.length > 0) {
            return res.status(400).json({ missatge: 'Aquesta amistat ja existeix' });
        }

        await connection.execute(`
      INSERT INTO AMISTATS (id_usuari, id_usuari_amic, estat)
      VALUES (?, ?, 'pendent')
    `, [id_usuari, id_usuari_amic]);

        res.status(200).json({ missatge: 'Sol·licitud d\'amistat enviada' });
    } catch (error) {
        console.error('Error afegint amic:', error);
        res.status(500).json({ missatge: 'Error intern del servidor' });
    }
};

// DELETE /amics/eliminar
exports.eliminarAmic = async (req, res) => {
    try {
        const { id_usuari, id_usuari_amic } = req.body;
        const connection = await connectDB();

        const [result] = await connection.execute(`
      DELETE FROM AMISTATS
      WHERE (id_usuari = ? AND id_usuari_amic = ?)
         OR (id_usuari = ? AND id_usuari_amic = ?)
    `, [id_usuari, id_usuari_amic, id_usuari_amic, id_usuari]);

        if (result.affectedRows === 0) {
            return res.status(404).json({ missatge: 'No s\'ha trobat l\'amistat' });
        }

        res.status(200).json({ missatge: 'Amistat eliminada correctament' });
    } catch (error) {
        console.error('Error eliminant amic:', error);
        res.status(500).json({ missatge: 'Error intern del servidor' });
    }
};
...
exports.acceptarAmistat = async (req, res) => {
    ...
    `, [id_usuari_amic, id_usuari]); // L'amic original va enviar la sol·licitud

        if (result.affectedRows === 0) {
            return res.status(404).json({ missatge: 'No s\'ha trobat la sol·licitud pendent' });
        }

        res.status(200).json({ missatge: 'Amistat acceptada correctament' });
    } catch (error) {
...
// DELETE /amics/rebutjar
exports.rebutjarAmistat = async (req, res) => {
    try {
        const { id_usuari, id_usuari_amic } = req.body;
        const connection = await connectDB();
      ...
      DELETE FROM AMISTATS
      WHERE id_usuari = ? AND id_usuari_amic = ? AND estat = 'pendent'
    `, [id_usuari_amic, id_usuari]); // Només qui rep la sol·licitud pot rebutjar

        if (result.affectedRows === 0) {
            return res.status(404).json({ missatge: 'No s\'ha trobat la sol·licitud pendent' });
        }

        res.status(200).json({ missatge: 'Sol·licitud rebutjada correctament' });
    } catch (error) {
        console.error('Error rebutjant amistat:', error);
        res.status(500).json({ missatge: 'Error intern del servidor' });
    }
};
