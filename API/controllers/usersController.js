...
// Obtenir la quantitat d'exp de l'usuari
exports.get_exp = async (req, res) => {
    try {
        const connection = await connectDB();
        const [rows] = await connection.execute(
            'SELECT exp_emmagatzemada, nom FROM USUARIS WHERE nom = ?',
            [req.params.nom]
        );
        if (rows.length === 0) {
            return res.status(404).send("No s'ha trobat cap usuari amb aquest nom.");
...
// Actualitzar l'exp de l'usuari
exports.sumar_exp = async (req, res) => {
    try {
        const connection = await connectDB();
        const userId = req.params.id;
        const { nom } = req.body; // Nombre del enemic

        // Obtener los punts_donats del enemic
        ...
        const puntsEnemic = rowsEnemic[0].punts_donats;

        // Sumar puntsEnemic a la exp_emmagatzemada del usuari
        await connection.execute(`
            UPDATE USUARIS
            SET exp_emmagatzemada = exp_emmagatzemada + ?
            WHERE usuari = ?
        `, [puntsEnemic, userId]);

        await connection.end();

        res.status(200).json({ missatge: 'Experiència sumada', punts_sumats: puntsEnemic });
    } catch (err) {
        console.error(err);
