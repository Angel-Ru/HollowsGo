...
exports.updateFavoritePersonatge = async (req, res) => {
    try {
        const { userId } = req.params;
        const { personatge_preferit, skin_preferida } = req.body;

        if (!personatge_preferit || !skin_preferida) {
            return res.status(400).send('Dades incorrectes: Personatge o skin preferit mancant');
        }
        ...
        res.send('Personatge i skin preferits actualitzats correctament');
    } catch (err) {
        console.error(err);
        res.status(500).send('Error en la consulta');
    }
